...
- Keep unit tests in `test/ash_agent_tools`, mirroring `lib/` structure with `<filename>_test.exs`, and default to `async: true` when isolation is possible.
...
```
test/
├── ash_agent_tools/           # Unit tests (mirror lib/ structure)
...
defmodule AshAgentTools.Integration.Metadata.ReqLLM.OpenAITest do
  use AshAgentTools.IntegrationCase, backend: :req_llm, provider: :openai
